...
- 앞서 추출된 특징(feature)을 이용해 최종 분류나 예측을 수행하는 단계
- CNN의 합성곱 + 풀링 계층이 이미지에서 유의미한 특징을 추출했다면 FC 계층은 이 특징들을 기반으로 클래스를 판별하거나 회귀 예측을 수행
```
입력 이미지
 ↓
Conv → ReLU → Pooling → ...
...
| 항목    | 설명                                                         |
| ----- | ---------------------------------------------------------- |
|  구조 | 모든 입력 뉴런이 모든 출력 뉴런과 연결됨                                    |
|  입력 | 이전 단계의 **특징맵을 펼친(flattened)** 1차원 벡터                       |
|  목적 | **분류/회귀 등의 최종 예측 결과 생성**                                   |
|  예  | MNIST 숫자 분류: 10개의 클래스 출력 → `[0.01, 0.03, ..., 0.85, 0.02]` |
| ⚠ 단점 | **매우 많은 파라미터** 발생 → 과적합 위험, 계산량 ↑                          |

### FC와 CNN 비교
| 항목     | CNN (Conv/Pooling) | FC Layer  |
| ------ | ------------------ | --------- |
| 구조     | 부분 연결              | 완전 연결     |
| 파라미터 수 | 적음                 | 많음        |
| 역할     | 특징 추출              | 최종 판단     |
| 장점     | 공간 구조 보존           | 고차원 결합 가능 |
